...
int main(int argc, char* argv[]) {
	...
	while(!game_over) {
		...
		// changed the previous black and white to better match the state of our board
		if (game.turn_white()) {
		...
		if (choice.length() != 1) {
			...
			case 'L': case 'l': {				
				...
				try {
					ifs >> game;
				} catch(Chess::Exception const& ex) {
			...
			case 'M': case 'm': {
				...
				// And make the move
		  			...
		  			try {
						game.make_move(std::make_pair(toupper(argument[0]), toupper(argument[1])),
								   std::make_pair(toupper(argument[2]), toupper(argument[3])));
					} catch(Chess::Exception const& ex) {
						std::cerr << ex.what() << std::endl;
					}
